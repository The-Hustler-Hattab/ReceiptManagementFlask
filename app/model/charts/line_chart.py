...
class LineChart:
    ...
    @staticmethod
    def get_by_field(charts: List['LineChart'], category: str) -> List[str]:
        chart_list = []
        for chart in charts:
    ...
    @staticmethod
    def get_labels(charts: List['LineChart']) -> List[str]:
        chart_list = []
        for chart in charts:
    ...
    @staticmethod
    def get_data_all(chart_models: List['LineChart']) -> Dict[str, Dict]:
        labels = LineChart.get_labels(chart_models)

        data_chart_model_dict: Dict[str, Dict] = {}
        data_chart_model_dict['total_amount'] = {"data": LineChart.get_by_field(chart_models, 'total_cost'),
                                                 "label": labels, "borderColor": '--red-500'}

        data_chart_model_dict['total_amount_subtotal'] = {
            "data": LineChart.get_by_field(chart_models, 'total_sub_total'),
            "label": labels, "borderColor": '--blue-500'}

        data_chart_model_dict['total_tax'] = {"data": LineChart.get_by_field(chart_models, 'total_tax'),
